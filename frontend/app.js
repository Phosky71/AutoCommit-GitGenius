...
async function toggleTimer() {
    if (timerRunning) {
        try { await invoke('stop_auto_commit'); setTimerState(false); toast('Timer stopped', 'info'); }
        catch (e) { toast('Failed to stop timer: ' + e, 'error'); }
    } else {
        try { await invoke('start_auto_commit'); setTimerState(true); toast('Timer started', 'success'); }
...
<div class="repo-card" id="repo-card-${repo.id}">
  ...
  <div class="repo-meta">
    ...
    <div class="repo-meta-item">
      ...
      Last: ${formatRelTime(repo.last_commit_time)}
    </div>` : ''}
...
/* ── EVENT DELEGATION FOR DYNAMIC REPO CARDS ─────────────── */
...
async function previewDiff(path, targetId) {
    try {
        ...
        el.innerHTML = `
            <span class="diff-files">${stats.files_changed} file${stats.files_changed !== 1 ? 's' : ''}</span>
...
async function previewRepoDiff(path) {
    ...
    try {
        ...
        if (!stats || stats.files_changed === 0) {
            ...
            box.innerHTML = `${stats.files_changed} file(s) changed &nbsp;<span class="diff-add">+${stats.insertions}</span> <span class="diff-del">-${stats.deletions}</span> &nbsp;~${stats.estimated_tokens} tokens`;
        }
...
async function runDryRun(path) {
    try {
        ...
        const handler = () => {
            closeDryRunModal();
            commitNowByPath(path);
            commitBtn.removeEventListener('click', handler);
        };
        commitBtn.removeEventListener('click', commitBtn._dryRunHandler);
        commitBtn._dryRunHandler = handler;
        commitBtn.addEventListener('click', handler);
    } catch (e) {
        document.getElementById('dryrun-result').textContent = 'Error: ' + e;
    }
}
...
async function loadSettings() {
    try {
        ...
        if (sel) sel.value = cfg.provider || 'lm_studio';
        ...
        if (cfg.auto_start) {
            try { await invoke('start_auto_commit'); setTimerState(true); }
            catch (e) {}
        }
...
function updateHistoryRepoFilter() {
    ...
    sel.innerHTML = '<option value="">All repositories</option>' +
        paths.map(p => {
            ...
            return `<option value="${escHtml(p)}" ${p === cur ? 'selected' : ''}>${escHtml(name)}</option>`;
        }).join('');
}
...
<div class="history-item">
  ...
  <div class="history-meta">
    ...
    <div class="history-meta-item diff-files">${entry.files_changed} file(s)</div>
...
async function init() {
    ...
    document.getElementById('btn-add-repo')
        .addEventListener('click', openAddRepoModal);
    document.getElementById('btn-add-repo-empty')
        .addEventListener('click', openAddRepoModal);

    document.getElementById('btn-close-repo-modal')
        .addEventListener('click', closeRepoModal);
    document.getElementById('btn-cancel-repo-modal')
        .addEventListener('click', closeRepoModal);
    document.getElementById('btn-save-repo')
        .addEventListener('click', saveRepo);
    document.getElementById('btn-browse-repo')
        .addEventListener('click', pickDirectory);
    document.getElementById('repo-path')
        .addEventListener('click', pickDirectory);
    document.getElementById('repo-push-enabled')
        .addEventListener('change', togglePushBranch);
    ...
    document.getElementById('btn-close-dryrun-modal')
        .addEventListener('click', closeDryRunModal);
    document.getElementById('btn-cancel-dryrun')
        .addEventListener('click', closeDryRunModal);

    document.getElementById('btn-save-settings')
        .addEventListener('click', saveSettings);
    document.getElementById('cfg-provider')
    ...
    document.getElementById('history-filter-repo')
        .addEventListener('change', renderHistory);
    document.getElementById('history-filter-type')
        .addEventListener('change', renderHistory);

    document.addEventListener('keydown', e => {
        if (e.key === 'Escape') { closeRepoModal(); closeDryRunModal(); }
    });
